...
const DetailsCard = ({ listing, isOwner, mutate }: Props) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mutate) {
        mutate((prev: any) =>
          prev ? { ...prev, [editData.key]: value } : prev,
        );
      }
  ...
  const renderValue = (
    ...
    value: any,
    fieldKey: string,
    textTransform: any = "none",
  ) => {
    const displayValue = value || t("details.na");
    ...
    return (
      ...
      >
        <Typography
          ...
          title={`${t(labelKey)}: ${displayValue}`}
        >
          {t(labelKey)}:{" "}
          <strong
            style={{ textTransform, cursor: value ? "pointer" : "default" }}
            onClick={handleCopy}
          >
            {displayValue}
          </strong>
        ...
        {isOwner && (
          <IconButton
            size="small"
            onClick={() => handleEditClick(fieldKey, t(labelKey), value)}
          >
  ...
  return (
    <>
      <Grid container spacing={1} sx={{ mt: 2, mb: 0.5 }}>
      ...
      <Grid container spacing={0.5}>
        <Grid size={{ xs: 12, sm: 6 }}>
          {renderValue(
            "details.fuelType",
            listing.fuelType
              ? t(`carValues.fuel_${listing.fuelType}`, {
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          {renderValue(
            "details.drivetrain",
            listing.drivetrain
              ? t(`carValues.drivetrain_${listing.drivetrain}`, {
                  defaultValue: listing.drivetrain,
                })
              : t("details.na"),
            "drivetrain",
          )}
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          {renderValue(
            ...
            listing.transmission
              ? t(`carValues.transmission_${listing.transmission}`, {
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          {renderValue(
            ...
            listing.interiorColor
              ? t(`carValues.color_${listing.interiorColor}`, {
                  defaultValue: listing.interiorColor,
                })
              : t("details.na"),
            "interiorColor",
          )}
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          {renderValue(
            "details.color",
            listing.color
              ? t(`carValues.color_${listing.color}`, {
      ...
      <Dialog
        ...
        onClose={() => !busy && setEditData(null)}
        maxWidth="xs"
        fullWidth
      >
        <DialogTitle>
          {t("common.edit", { defaultValue: "Edit" })} {editData?.label}
        </DialogTitle>
